  ...
  return `<div class="flex min-h-0 min-w-0 flex-1 flex-col bg-primary-light dark:bg-primary-dark">${validate_component(Container, "Container").$$render($$result, { custom: "!pt-2 sm:!pt-3" }, {}, {
    default: () => {
      return `${validate_component(Heading, "Heading").$$render(
        ...
        {},
        {}
      )} <div class="flex min-h-0 min-w-0 flex-1 flex-col"><div role="list" class="-mt-1 grid min-h-0 min-w-0 flex-1 grid-cols-1 overflow-x-clip sm:-mt-0">${`<div class="col-start-1 row-start-1 z-[1] flex min-h-0 min-w-0 flex-1 flex-col gap-0.5 self-stretch justify-self-stretch"><p class="animate mb-1.5 font-sans text-xs font-medium uppercase tracking-widest text-neutral-500 dark:text-neutral-400" data-svelte-h="svelte-2nbdoa">Featured</p> ${each(featuredProjects, (project) => {
      ...
      })} <div class="animate mt-3 w-full sm:mt-4"><div class="flex flex-col items-center gap-0.5"><button type="button" class="font-sans inline-flex cursor-pointer items-center justify-center gap-2 border-0 bg-transparent px-0 py-0.5 text-xs font-normal text-neutral-500 no-underline transition-colors duration-100 ease-out hover:text-neutral-950 sm:text-sm dark:text-neutral-400 dark:hover:text-white"${add_attribute("aria-expanded", showAll, 0)}><span data-svelte-h="svelte-1o7ae2r">Browse everything</span> <svg class="h-4 w-4 transition-transform duration-200 ease-out rotate-0" fill="none" stroke="currentColor" viewBox="0 0 24 24" xmlns="http://www.w3.org/2000/svg" aria-hidden="true"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M19 9l-7 7-7-7"></path></svg></button></div></div> ${validate_component(Footer, "Footer").$$render($$result, {}, {}, {})}</div>`}</div></div>`;
    }
  })}</div>`;
});
export {
